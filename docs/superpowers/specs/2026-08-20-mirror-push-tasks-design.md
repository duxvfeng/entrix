...
- 带 tag 的任务调用 `scripts/push_current_tag.py`。脚本读取指向 `HEAD` 的本地 tag，
  没有 tag 或存在多个候选 tag 时失败；它只推送已有 tag，不执行 `git tag`。
...
- 当前 `HEAD` 没有唯一 tag 时，tag 推送任务在本地失败并显示原因，不产生远端变更。
- 任一 remote 推送失败时，双 remote 任务按顺序停止，不掩盖失败。

## 验证

- 单元测试覆盖当前 tag 的唯一性、无 tag 和多 tag 失败路径，以及实际推送命令参数。
